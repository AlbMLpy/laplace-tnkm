...
from source.models.LaplaceBNN import LaplaceBNN
...
MODEL_HELP = "Choose model: 'la_btn', 'mf_btn', 'sp_btn', 'la_bnn';"
...
FMAP_HELP = "Choose feature mapping:\
    'poly', 'poly_norm', 'fourier', 'alt' (la_bnn);"
...
def argparse_uci():
    ...
    parser.add_argument(
        '-fs', '--f_scale', type=float, default=1.0, help=SCALE_HELP)
    parser.add_argument(
        '-fsh', '--f_shift', type=float, default=0.0, help=SHIFT_HELP)
    parser.add_argument(
        '-s', '--scorer', type=str, default='ll', help=SCORER_HELP)
    parser.add_argument(
...
def get_res_dir(args, dir_path=ART_DIR):
    ...
    return Path(
        (
            dir_path 
            / f'{args.model}/{args.data}/{args.hess}/{args.scorer}/{fmap_str}/'
        )
    )
...
def get_model_spec(model: str):
    if model == 'la_btn':
        ...
        return LaplaceBNN, get_exp_config_la_bnn
    else:
...
if __name__ == '__main__':
    ...
    load_data_f = partial(
        load_transform_data, data_path=data_path, test_size=TEST_SIZE,
        transform_x=TRANSFORM_X, transform_y=TRANSFORM_Y, scaler=SCALER,
    )
    model_cls, config_f = get_model_spec(args.model)
    if args.model == 'la_bnn':
        ptm_func = prepare_train_model_simple
        jax.config.update("jax_enable_x64", False)
    else:
        ptm_func = prepare_train_model
    train_model_f = ptm_func(
        config_f(
            model_cls, n_samples, d_dim, data_path, args.hess, 
            get_fmap(args.fmap, args.f_scale, args.f_shift),
            get_scorer(args.scorer), args.tqdm_enable,
        ),
        logger=logger,
    )
    get_stats_several_trials(
